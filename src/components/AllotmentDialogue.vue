...
export default {
	...
	watch: {
		room_id: function(prevval, newval) {
			do {
				this.hostel = this.hostelRooms.find(
					(e) => e.room_id === this.room_id
				);
			...
			if (!this.isOccupant2Disabled) {
				this.occupant2list = this.students.filter(
					(student) =>
						student.username !== this.occupant1 &&
			...
			if (!this.isOccupant1Disabled) {
				this.occupant1list = this.students.filter(
					(student) =>
						student.username !== this.occupant2 &&
						student.is_alloted === 0
				);
			}
		},
		...
		dialog: function() {
			...
			axios
				.get("http://localhost:8081/server/admin/hostels")
				.then((res) => {
